...
function getDates() {
  ...
  console.log(attdData[0].students.length);
  if (window.location.pathname == "/student/") {
    testAtdData = packAllAtd();
    makeAttendanceAreaChart();
  } else if (window.location.pathname == "/coach/") {
    let inputDataSet = [];
    for (let i = 0; i < attdData[0].students.length; i++) {
      inputDataSet.push(makeDataset("Test", packAllAtd(i)));
    }
    console.log([inputDataSet]);
    makeAttendanceTotalChart(inputDataSet);
    packageLatenessData();
  }
...
function packageStudentAttendanceData(targetStudent, callback) {
  ...
  for (let i = 0; i < attdData.length; i++) {
    let timeInStr =
      attdData[i].date + " " + attdData[i].students[targetStudent].timeIn;
    let timeOutStr =
      attdData[i].date + " " + attdData[i].students[targetStudent].timeOut;

    timeInStr = timeInStr.replace(/_/g, "/");
    timeOutStr = timeOutStr.replace(/_/g, "/");

    let timeIn = new Date(
      getDateStrFromIndex(i) + " " + attdData[i].students[targetStudent].timeIn
    );
    let timeOut = new Date(
      getDateStrFromIndex(i) + " " + attdData[i].students[targetStudent].timeOut
    );
...
function packageLatenessData() {
  ...
  for (let i = 0; i < 3; i++) {
    for (var key in excuses) {
      arr[i].push(
        percentage(excuses[key][excuseStrings[i]], excuses[key]["total"])
      );
    }
  }
...
function addDayToData(data_text, index) {
  ...
  if (dateCounter == dateCounterTarget) {
    attdData.push([]);
    httpGetAsync(
      "http://127.0.0.1:5000/static/piData/Attendance.json",
      addCurrentDay
    );
  }
}
    ...
    for (let j = 0; j < attdData[i].reason.length; j++) {
      console.log(
        i +
          ...
          reasonDict[j] +
          ": " +
          data[i].reason[j].count +
          "  " +
          data[i].reason[j].percent +
          "%"
      );
    }
  }
